...
public class HandAnimator : MonoBehaviour
{
    ...
    HandPresence[] controllerPresence;
...
readonly List<Finger> gripFingers = new List<Finger>()
    ...
    private void Update()
    {
        ...
        if (controllerPresence.Length <2) { controllerPresence = FindObjectsOfType<HandPresence>(); }


        if(showcontroller) //this is ugly and not preformant, references prefab containing HandPresence script that gets instantiated into 
                           //actionbasedcontroller script serialized field (model prefab): this can be combined with present script later on
        {
            foreach (HandPresence h in controllerPresence)
            {
                h.showController = true;
            }   
    ...
    void TryInitialize()
    {
        ...
        controllerPresence = FindObjectsOfType<HandPresence>();
